...
class GasTable {
   ...
   */
  generate(info, info2) {
    colors.enabled = false; //!this.config.noColors || false;
    ...
    _.forEach(info.methods, (data, methodId) => {
      ...
      if (!this.config.onlyCalledMethods || data.numberOfCalls > 0) {
        ...
        const diff = (stats2.average / stats.average - 1) * 100;
        if (diff !== 0) {
          ...
          section.push({
            hAlign: 'right',
            content: colors.bold(`${diff.toFixed(2)} %`),
          });
    ...
    console.log(`Optimizer enabled: ${solc.optimizer}`);
    console.log(`Runs: ${solc.runs}`);
    console.log(`Block limit: ${info.blockLimit} gas`);
    console.log(`\n`);
    ...
    if (this.config.ethPrice && this.config.gasPrice) {
      ...
      methodSubtitle = [
        { hAlign: 'left', colSpan: 2, content: colors.green.bold('Methods') },
        { hAlign: 'center', colSpan: 3, content: colors.green.bold('before') },
        { hAlign: 'center', colSpan: 3, content: colors.green.bold('after') },
      ];
    }
    ...
    if (methodRows.length == 0 && deployRows.length == 0) {
      table.push([
        {
          hAlign: 'left',
          colSpan: 1,
          content: colors.green.bold('⛽️ Gas consumption was not changed!'),
        },
      ]);
    } else {
