...
class GasTable {
  ...
  generate(info, info2) {
    ...
    _.forEach(Object.entries(info.methods), ([methodId, data], index) => {
      if (!data) return;

      const data2 = Object.values(info2.methods).find(
        (x) => (x.key = methodId.split('_')[1]),
      );
    ...
    info.deployments.forEach((contract, index) => {
      ...
      console.log(Object.keys(info2.deployments));

      const contract2 = info2.deployments[index];
      let stats = {};
      let stats2 = {};

      if (!contract.gasData.length || !contract2 || !contract2.gasData.length) return;
